"""
Build the QQQ index dataset for backtest_dataset/INDICES/QQQ/{5m,10m,1h,1d}.
...
Output:
    backtest_dataset/INDICES/QQQ/{5m,10m,1h,1d}/qqq_full_dataset.parquet

Usage (from backtester_api/):
    python -m scripts.build_qqq_dataset              # all timeframes
    python -m scripts.build_qqq_dataset --timeframe 5m
    python -m scripts.build_qqq_dataset --timeframe 10m
    python -m scripts.build_qqq_dataset --timeframe 1h
    python -m scripts.build_qqq_dataset --timeframe 1d
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build QQQ dataset (5m/10m/1h/1d, last 5 years) in backtest_dataset/INDICES/QQQ/."
    )
    ...
    # When building all, reuse in-memory DataFrames for the joins
    if tf is None:
